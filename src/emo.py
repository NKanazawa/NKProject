...
def selFinal(individuals, k, nd='Total'):
    """
    :param individuals: A list of individuals to select from.
    :param k: The number of individuals to select.
    :param nd: Specify the non-dominated algorithm to use: 'standard' or 'log'.
...
def sortNondominated(individuals, k, first_front_only=False):
    ...
    for i, fit_i in enumerate(fits):
        for ind_j, fit_j in zip(inds[i + 1:], fits[i + 1:]):
            if inds[i][0].isFeasible == True and ind_j[0].isFeasible == False:
                dominating_fits[fit_j] += 1
                dominated_fits[fit_i].append(fit_j)
                continue
            elif inds[i][0].isFeasible == False and ind_j[0].isFeasible == True:
                dominating_fits[fit_i] += 1
                dominated_fits[fit_j].append(fit_i)
                continue
            if inds[i][0].isFeasible == False and ind_j[0].isFeasible == False and (inds[i][0].volViolation < ind_j[0].volViolation):
                dominating_fits[fit_j] += 1
                dominated_fits[fit_i].append(fit_j)
                continue
            elif inds[i][0].isFeasible == False and ind_j[0].isFeasible == False and (inds[i][0].volViolation > ind_j[0].volViolation):
                dominating_fits[fit_i] += 1
                dominated_fits[fit_j].append(fit_i)
                continue
