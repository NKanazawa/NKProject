...
def zdt1(LOWBOUNDS, UPBOUNDS, ind):
    ...
    for i in range(1, N):
        if i % 2 == 0:
            obj1 += 2 * numpy.power(gnm[i] - 0.8 * gnm[0] * numpy.cos(6 * numpy.pi * gnm[0] + ((i + 1) * numpy.pi / N)),
                                    2) - numpy.cos(4 * numpy.pi * (
                        gnm[i] - 0.8 * gnm[0] * numpy.cos(6 * numpy.pi * gnm[0] + ((i + 1) * numpy.pi / N)))) + 1
        elif i == 1:
            if gnm[i] - 0.8 * gnm[0] * numpy.sin(6 * numpy.pi * gnm[0] + ((i + 1) * numpy.pi / N)) < (3 / 2) * (
                    1 - (numpy.sqrt(2) / 2)):
                obj2 += numpy.abs(gnm[i] - 0.8 * gnm[0] * numpy.sin(6 * numpy.pi * gnm[0] + ((i + 1) * numpy.pi / N)))
            else:
                obj2 += 0.125 + numpy.power(
                    gnm[i] - 0.8 * gnm[0] * numpy.sin(6 * numpy.pi * gnm[0] + ((i + 1) * numpy.pi / N)) - 1, 2)
        else:
            obj2 += 2 * numpy.power(gnm[i] - 0.8 * gnm[0] * numpy.sin(6 * numpy.pi * gnm[0] + ((i + 1) * numpy.pi / N)),
                                    2) - numpy.cos(4 * numpy.pi * (
                        gnm[i] - 0.8 * gnm[0] * numpy.sin(6 * numpy.pi * gnm[0] + ((i + 1) * numpy.pi / N)))) + 1
    result = [obj1, obj2]
    g1 = gnm[1] - 0.8 * gnm[0] * numpy.sin(6 * numpy.pi * gnm[0] + (2 * numpy.pi / N)) - 0.5 * gnm[0] + 0.25
    conresult = [g1]
